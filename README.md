...
AML 업무를 처음 접하는 사람도 화면만 보고 따라갈 수 있도록, 룰마다
**무엇을 탐지하는지·왜 의심스러운지·근거 조문**을 함께 표시하고 사이드바에
용어 설명(알림·진성·정밀도·STR)과 화면을 보는 순서를 두었습니다.
③ 알림 상세는 판정 근거를 원시 JSON으로 던지지 않고 문장으로 풀어 보여주며,
원시 수치는 따로 열어 볼 수 있게 했습니다.
...
④항 거래 표는 **최대 100건까지만 싣고** 초과분은 건수만 표기합니다.
자금통과계좌는 근거 거래가 최대 82,841건에 이르러 전량을 펼치면 문서가
6.7MB로 불어나 사람이 읽을 수 없습니다. 다만 거래 건수와 합산 금액은
생략분을 포함한 전체 기준을 유지하고, ⑥항에 전체 건수와 발췌 사실을 적어
누락을 감추지 않습니다.

### 종합의견 작성 원칙

FIU 심사분석 사례가 제시하는 작성 원칙을 ⑤항 구조에 반영했습니다.
**육하원칙**에 따라 기재하고, 아래 8개 항목을 포함합니다.

| 항목 | 채우는 주체 |
|---|---|
| 명확한 보고 이유 | 담당자 — 전제범죄 유형을 가늠할 수 있게 |
| 창구 정황 | 담당자 — 고객 진술, 질문 회피·진술 번복·대리인 개입 |
| 명확한 고객 정보 | 담당자 — 직업·업종, 법인이면 대표자와의 관계 |
| 관련인 | 담당자 — 가족·법인 임직원 등 관계인 |
| 송금자 정보 | 담당자 — 전신송금 시 송금인·수취인과 목적 |
| 자금 출처 | 담당자 — 원천과 확인 근거 |
| **변동된 차이점** | **시스템(정량) + 담당자(질적 차이)** |
| 특이점 | 담당자 — 소득 대비 규모 불일치, 과거 STR 이력 |

**이 설계의 핵심은 자동 생성의 한계를 명시하는 데 있습니다.** 심사분석
미흡 사례는 *"다수 계좌를 통한 분할입금 및 지급이어서 보고함"* 처럼 탐지
조건을 그대로 옮겨 적은 보고로, "계좌주 정보와 창구 현장정보가 없어
심사분석에 별다른 도움이 되지 않았다"고 평가됩니다. 룰 기반 시스템이
자동으로 쓸 수 있는 문장이 정확히 이 형태이므로, 초안은 시스템 판정을
**탐지 근거일 뿐 보고 사유가 아니라고 명시**하고 담당자 기재란으로
유도합니다.

'변동된 차이점'은 정량 비교만 시스템이 채웁니다. 또 다른 미흡 사례가
"1회 이체 정황만 기술하고 전후 거래유형 분석이 없다"는 평가를 받은 점을
반영해, 알림 구간의 거래빈도를 그 계좌의 평소 빈도와 대조합니다.

```
알림 구간 5.0일간 82,841건(일평균 16,568.2건),
계좌 전체 관측구간 10.0일간 169,756건(일평균 16,976.8건)
→ 알림 구간 거래빈도는 평소의 0.98배, 계좌 전체 거래의 48.8%
```

위 예시는 알림 구간이 평소와 거의 다르지 않음을 보여줍니다 — 심사역이
오탐을 조기에 걸러낼 근거가 됩니다. 비교에 쓰는 계좌별 평소 거래 프로파일
(`data/alerts/account_profile.parquet`)은 원본 이력을 싣지 않고 집계값만
담아 280KB입니다(전체 이력을 담으면 20.3MB).
...
# 7. 배포용 거래 샘플 + 계좌 프로파일 갱신 (룰 재실행 시 함께 수행)
...
> `python src/make_tx_sample.py`로 샘플과 계좌 프로파일을 함께 갱신해야 합니다.
...
│   ├── make_tx_sample.py       # 배포용 거래 샘플 + 계좌 프로파일 생성
...
- 데이터 기간이 18일이라 계좌 연령·휴면 여부 기반 탐지는 구현할 수
  없습니다.
- 같은 이유로 STR '변동된 차이점'의 "평소 거래" 표본이 짧습니다. 계좌별
  관측구간이 9~11일 수준이어서, 실무에서 요구하는 수개월치 거래유형과의
  비교가 아니라 18일 창 안에서의 상대 비교에 그칩니다. 계절성이나 장기
  추세에서 벗어난 변화는 이 지표로 잡히지 않습니다.
